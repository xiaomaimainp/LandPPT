...
class PromptTemplates:
    ...
    def get_structure_analysis_prompt() -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate([
            ...
            **重要说明：**
            - 识别文档原始语言仅用于参考，实际生成内容的语言将由用户在表单中选择决定
            - 提取具体的数据、数字、百分比等关键信息
            - 识别文档的核心结论和重要观点
    ...
    def get_initial_outline_prompt() -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate([
            ...
            **要求：**
            ...
            2. **内容丰富性与创意性**：每个幻灯片的content_points要充分发挥AI的创意和设计能力：
               ...
               - 结合行业趋势、前沿技术、创新思维
               - 提供多角度分析，展现内容的深度和广度
            3. **图表样式多样化**：在适当的内容页中添加chart_config字段，支持丰富的Chart.js图表样式：
        ...
        return ChatPromptTemplate([
            ...
            **要求：**
            ...
            2. **内容深度挖掘与创意表达**：细化内容时要充分发挥AI的创意和分析能力：
               ...
               - 融入生动的案例、实例、对比研究、成功故事
               - 引用权威观点、专家见解、行业报告、研究成果
            ...
            - 确保每张幻灯片内容量适中（3-6个要点），但内容要充实有深度
            - 维护逻辑顺序和演示流程，增强内容的连贯性和说服力
            - 添加具体数据、深度结论和生动案例，避免空泛的概念性描述
            - 充分发挥AI的创意能力，生成富有洞察力和表现力的内容
    ...
    def get_finalize_outline_prompt() -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate([
            ...
            **最终优化严格要求：**
            ...
            4. 确保每张幻灯片内容量适中但信息丰富：
               - 标题页：3-5个要点（包含主题亮点、价值主张）
               - 内容页：3-6个要点（包含数据、结论、洞察、案例）
               - 结论页：3-5个要点（包含总结、启示、行动建议）

            5. 添加必要的过渡和总结幻灯片，增强逻辑连贯性
            ...
            返回最终的JSON格式PPT大纲，包含：
            - title: 最终PPT标题（使用目标语言：{target_language}）
            - total_pages: 最终总页数
