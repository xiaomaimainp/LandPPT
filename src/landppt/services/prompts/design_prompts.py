...
**设计哲学**：
1. **模板框架绝对保持原则** - 页眉和页脚区域必须与参考模板完全一致，不允许任何修改
2. **一致性原则** - 严格遵循核心设计基因，确保品牌识别度和视觉统一性
3. **创新性原则** - 仅在主要内容区域内进行创新，避免千篇一律但不破坏模板框架
4. **内容适配原则** - 让设计服务于内容，但必须在模板框架约束内进行
5. **用户体验原则** - 优化信息传达效率和视觉舒适度，同时保持模板的专业性

**创意要求**：
- **严格约束**：页眉和页脚区域绝对不允许任何创意修改，必须与参考模板完全一致
...
- **页眉页脚严格保持原样**：
  ...
  * 不允许修改页眉页脚的任何视觉属性（颜色、字体、大小、位置、边距等）
  * 只能修改页眉和页脚之间的主要内容区域
- **空间利用优化**：
...
4. **内容适配原则** - 让设计服务于内容，但必须在模板框架约束内进行
5. **用户体验原则** - 优化信息传达效率和视觉舒适度，同时保持模板的专业性
6. **空间最大化原则** - 在主要内容区域内充分利用页面空间，避免大量留白浪费
   * 内容应该垂直分布均匀，占满主要内容区域的可用高度
   * 使用合适的字体大小和间距，确保内容填充整个主要内容区域
   * 主要内容区域底部不应该有超过50px的空余空间
